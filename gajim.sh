...
(14:44:04) 111: В общем в принципе можно забить на тот факт что в unity у gajim половина функционала не работает, достаточно лишь, что бы ты мне обьяснил как написать скрипт, суть которого будет сводиться к элементарному
просто запуск команд в терминале (может сам нугуглю) последовательно.
Суть такая
в скрипте будет список комнат, в которые я буду заходить, когда его запускаю.
Пример
$ gajim-remote join_room kapiz@conference.jabber.ru ник xmpp.ru | gajim-remote join_room любовь@conference.jabber.ru ник xmpp.ru и так далее
[06:20:27 AM] Сергeй: xmpp.ru - это название учётной записи
